...
const _TYPE_TO_SPN_PT_CNTR_NAMES: Dictionary[SpawnPointType, String] = {
...
var _lvl_registry: Dictionary[LevelId, String] = {
	LevelId.LIMINAL_REACH : "uid://cknam78ta6oro"
}

var _crnt_lvl: Node2D
var _crnt_lvl_spn_pts_cntr: Node

func load_level_by_id(id: LevelId) -> void:
	if _crnt_lvl:
		_crnt_lvl.queue_free()
		
	if not _lvl_registry.has(id):
		push_error("Level registry does not contain level with id %s" % id)
		return
		
	var lvl_uid: String = _lvl_registry[id]
	...
	_crnt_lvl = lvl_scene.instantiate()
	_crnt_lvl_spn_pts_cntr = _crnt_lvl.find_child("SpawnPointsContainer")
	
	self.add_child(_crnt_lvl)
...
func load_random_level() -> void:
	...
	var lvl_keys: Array = _lvl_registry.keys()
	
	if lvl_keys.is_empty():
		push_error("Level registry is empty")
		return
	
	var lvl_uid: String = _lvl_registry[lvl_keys.pick_random()]
	var lvl_scene: PackedScene = load(lvl_uid)
...
func _cache_current_level_spawn_points() -> void:
	...
	if not _crnt_lvl_spn_pts_cntr:
		push_error("'SpawnPointsContainer' node not found in %s scene tree" % _crnt_lvl.name)
		
	for type in SpawnPointType.values():
		type_to_spn_pts[type] = []
		
		var node_name = _TYPE_TO_SPN_PT_CNTR_NAMES[type]
		var sub_spn_pts_cntr = _crnt_lvl_spn_pts_cntr.find_child(node_name)
		
		if not sub_spn_pts_cntr:
			push_error("Failed to find %s node in %s scene tree" % [node_name, _crnt_lvl.name])
